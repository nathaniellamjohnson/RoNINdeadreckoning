...
def predict(args, **kwargs):
    ...
    device = torch.device(args.device if torch.cuda.is_available() else 'cpu') # can be changed to gpu later if needed to accelerate

    # basic data gathering pulled from test function
    if args.predict_path is not None:
        if args.predict_path[-1] == '/':
            args.predict_path = args.predict_path[:-1]
        root_dir = osp.split(args.predict_path)[0]
        pred_data_list = [osp.split(args.predict_path)[1]]
    elif args.predict_list is not None:
        root_dir = args.data_dir if args.data_dir else osp.split(args.predict_list)[0]
        with open(args.predict_list) as f:
            pred_data_list = [s.strip().split(',')[0] for s in f.readlines() if len(s) > 0 and s[0] != '#']
    else:
        raise ValueError('Either predict_path or predict_path must be specified.')

    # Load the first sequence to update the input and output size
    _ = get_dataset(root_dir, [pred_data_list[0]], args, mode='predict')
    # CONFUSION POINT -> what exactly does this return & do I need to modify it to predict without ground truth?
    ...
    # UNSURE WHAT THIS DOES BUT ADDED TO MAKE THE PREDS WORK -- REMOVE AT WILL
    losses_vel = MSEAverageMeter(2, [1], _output_channel)
    ate_all, rte_all = [], []
    pred_per_min = 200 * 60
    ...
    for idx, data in enumerate(pred_data_list):
        assert data == osp.split(seq_dataset.data_path[idx])[1]
...
if __name__ == '__main__':
    ...
    # predict
    predict_cmd = mode.add_parser('predict')
    predict_cmd.add_argument('--predict_path', type=str, default=None)
